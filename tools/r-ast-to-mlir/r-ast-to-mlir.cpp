...
class AstLowerer {
public:
  AstLowerer(MLIRContext &ctx, ModuleOp module)
      : context(ctx), builder(&ctx), module(module) {}

  LogicalResult lowerModule(const llvm::json::Object &obj) {
...
private:
  ...
  LogicalResult emitError(llvm::StringRef message) {
    llvm::errs() << "r-ast-to-mlir: " << message << "\n";
    return failure();
  }
  ...
  void bind(llvm::StringRef name, Value value) {
  ...
  LogicalResult declareFunction(const llvm::json::Object &obj) {
  ...
  LogicalResult defineFunction(const llvm::json::Object &obj) {
    auto name = obj.getString("name");
    if (!name) return emitError("function missing 'name'");
  ...
  Type parseType(const llvm::json::Value &val) {
  ...
  Value parseExpr(const llvm::json::Object &obj) {
    auto kind = obj.getString("type");
    ...
    if (*kind == "Bool") {
      ...
      auto val = obj.get("value");
      bool v = val && val->getAsBoolean().value_or(false);
      return builder.create<r::ConstantOp>(builder.getUnknownLoc(), dtype,
                                           builder.getBoolAttr(v));
    }
    ...
    if (*kind == "Slice") {
      ...
      for (int64_t i = 0; i < rank; ++i) {
        ...
        if (auto *sizeObj = sizeVal.getAsObject()) {
          if (auto kind = sizeObj->getString("type")) {
            if (*kind == "Number") {
  ...
  Value coerceStoreValue(Value value, Type targetType) {
  ...
  LogicalResult parseIndexTarget(const llvm::json::Object &obj, Value &memref,
                                 SmallVector<Value, 4> &indices) {
    auto kind = obj.getString("type");
  ...
  LogicalResult parseStmt(const llvm::json::Object &obj) {
    auto kind = obj.getString("type");
...
int main(int argc, char **argv) {
  llvm::cl::opt<std::string> inputFilename(llvm::cl::Positional, llvm::cl::desc("<input json>"), llvm::cl::init("-"));
  ...
  AstLowerer lowerer(context, module);
